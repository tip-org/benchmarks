...
(define-fun-rec
  step
    ...
    (match x
      ...
      (case (Plus q r) (plus (step q y) (step r y)))
...
(assert-not
  (forall ((q R) (i Nat) (j Nat) (k Nat) (j2 Nat) (s (list A)))
    (=> (not (eps q))
      (= (recognise (conj (rep q i k) (rep q j j2)) s)
        (recognise (rep q (max2 i j) (min2 k j2)) s)))))
(check-sat)
