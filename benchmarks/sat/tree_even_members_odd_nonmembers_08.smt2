...
(define-fun-rec
  notMembers
    ...
    (match x
      (case nil true)
      (case (cons z xs) (and (not (member2 z y)) (notMembers xs y)))))
(define-fun-rec
  double
    ((x Nat)) Nat
    (match x
...
(define-fun-rec
  evens
    ...
    (match x
      ...
      (case (S n) (cons (double n) (evens n)))))
(define-fun-rec
  odds
    ((x Nat)) (list Nat)
    (match x
...
(assert-not
  (forall ((t (Tree Nat)))
    (or (not (balanced t))
      (or (not (ordered t))
        (or (not (members (evens (S (S (S (S (S (S (S (S Z))))))))) t))
          (not (notMembers (odds (S (S (S (S (S (S (S (S Z))))))))) t)))))))
(check-sat)
