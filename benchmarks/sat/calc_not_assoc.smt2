...
(define-fun-rec
  varIn
    ((x Nat) (y Expr)) Bool
    (match y
      (case (Var z) (equal x z))
      (case (Plus a b) (or (varIn x a) (varIn x b)))
      (case (Mul c b2) (or (varIn x c) (varIn x b2)))
      (case (Const x2) false)))
(assert-not
  (forall ((e Expr) (x Nat) (y Nat) (z Nat))
    (or
      (=
        (eval
          (cons x
            (cons (eval (cons y (cons z (as nil (list Nat)))) e)
              (as nil (list Nat))))
          e)
        (eval
          (cons (eval (cons x (cons y (as nil (list Nat)))) e)
            (cons z (as nil (list Nat))))
          e))
      (or (not (varIn Z e)) (not (varIn (S Z) e))))))
(check-sat)
