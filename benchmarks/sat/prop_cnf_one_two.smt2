...
(assert-not
  (forall ((e Prop))
    (or (not (cnf e))
      (or (eval (cons true (cons true (as nil (list Bool)))) e)
        (or (not (eval (cons false (cons true (as nil (list Bool)))) e))
          (or (not (eval (cons true (cons false (as nil (list Bool)))) e))
            (eval (cons false (cons false (as nil (list Bool)))) e)))))))
(check-sat)
