...
(define-funs-rec
  ...
  ((match x8
     ...
     (case Nil x9))))
(define-funs-rec
  ((or2 ((x15 bool) (x16 bool)) bool)) ((ite x15 x15 x16)))
(define-funs-rec
  ...
  ((match x10
     ...
     (case
       ...
       (match x11
         ...
         (case Y true))))))
(define-funs-rec
  ((and2 ((x17 bool) (x18 bool)) bool)) ((ite x17 x18 x17)))
(define-funs-rec
  ((eps ((x13 R)) bool))
  ((match x13
     (case default false)
...
(define-funs-rec
  ...
  ((match x14
     ...
     (case Eps Nil)
     (case (Atom a3) x14)
     (case (Plus p6 q5) (Plus (deeps p6) (deeps q5)))
     (case
       (Seq p7 q6)
       (ite (and2 (eps p7) (eps q6)) (Plus (deeps p7) (deeps q6)) x14))
     (case (Star p8) (deeps p8)))))
...
(assert-not
  (forall
    ((p9 R) (s (list A)))
    (= (recognise (Star p9) s) (recognise (Star (deeps p9)) s))))
(check-sat)
