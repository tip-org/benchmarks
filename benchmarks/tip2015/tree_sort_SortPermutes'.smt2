  ...
  ((match x24
     ...
     (case
       ...
       (match x25
         ...
         (case (S d3) (le d2 d3)))))))
(define-funs-rec
  ((par (a4) (flatten ((x12 (Tree a4)) (x13 (list a4))) (list a4))))
  ((match x12
     (case
       (Node p2 x14 q)
...
(define-funs-rec
  ((elem ((x15 Nat) (x16 (list Nat))) bool))
  ((match x16
     (case nil false)
     (case (cons y ys) (or2 (equal x15 y) (elem x15 ys))))))
(define-funs-rec
  ((delete ((x17 Nat) (x18 (list Nat))) (list Nat)))
  ((match x18
     (case nil x18)
     (case
       (cons y2 ys2)
...
(define-funs-rec
  ((isPermutation ((x9 (list Nat)) (x10 (list Nat))) bool))
  ((match x9
     (case nil (null x10))
     (case
       (cons x11 xs2)
...
(define-funs-rec
  ((add ((x19 Nat) (x20 (Tree Nat))) (Tree Nat)))
  ((match x20
     (case
       (Node p3 y3 q2)
       (ite
